...
// Cleanup unwanted menu items from user view
async function cleanupUserMenu() {
    console.log('🧹 User menu cleanup initiated...');
    
    if (!confirm('Apakah Anda yakin ingin membersihkan menu yang bermasalah?\n\nMenu yang akan dihapus:\n- Es Campur Segar\n- Jus Alpuket\n- Sate Ayam Spesial\n- Dan menu lainnya yang tidak sesuai')) {
        return;
    }
    
    // Show loading state
    const cleanupBtn = document.querySelector('.btn-cleanup-user');
...
function logout() {
    if (confirm('Apakah Anda yakin ingin logout?')) {
        try {
            ...
            // Redirect to login page with single correct path
            setTimeout(() => {
                console.log('🔄 Redirecting to login page...');
                
                // Use single correct path to avoid duplication
                window.location.href = '../';
            ...
            // Force redirect with single correct path
            setTimeout(() => {
                window.location.href = '../';
            }, 2000);
        }
